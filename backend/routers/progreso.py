...
from models import get_db, RespuestaAlumno, Pregunta
from schemas import TestResultado

router = APIRouter(prefix="/api/progreso", tags=["Indicadores de Progreso"])

# ENDPOINT 1: Ahora calcula la MEDIA HISTÓRICA REAL
@router.get("/alumno/{alumno_id}/tema/{tema_id}")
def obtener_indicador_tema(alumno_id: int, tema_id: int, db: Session = Depends(get_db)):
...
# ENDPOINT 2: El que guarda los resultados del test (se mantiene igual)
@router.post("/guardar-resultados")
def guardar_resultados(resultado: TestResultado, db: Session = Depends(get_db)):
