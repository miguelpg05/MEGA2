...
export default function TestListado() {
  ...
  // --- ATENCIÓN: PARA ESTE PROTOTIPO USAMOS EL ALUMNO ID = 1 ---
  const ALUMNO_ID = 1; 

  useEffect(() => {
    fetch(`https://backend-academia-kxx5.onrender.com/api/test/listado-progreso?alumno_id=${ALUMNO_ID}`)
      .then(res => res.json())
      .then(datos => {
  ...
  // Función para determinar el color del indicador basándonos en los fallos
  const obtenerColorIndicador = (fallos) => {
    if (fallos === null) return "bg-gray-100"; // Nunca realizado
    if (fallos <= 2) return "bg-[#bef264]"; // Aprobado (imita tu color verde)
    return "bg-[#f87171]"; // Suspendido (rojo)
  };
  ...
  if (cargando) {
    return <div className="p-8 text-center text-orange-500 font-medium animate-pulse">Cargando el listado completo de tests...</div>;
  }
    ...
    <div className="min-h-screen bg-gray-50 p-6 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 overflow-x-auto">
        ...
        <header className="mb-8 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Listado de Tests</h1>
            <p className="text-gray-500 mt-1">Tests de 10 preguntas por tema.</p>
          </div>
          <button 
            onClick={() => navigate('/')}
            className="text-sm bg-gray-100 text-gray-600 px-5 py-2.5 rounded-xl hover:bg-gray-200 transition-colors"
          >
            ← Volver al Dashboard
          </button>
        </header>

        {/* --- TABLA PRINCIPAL (ESTILO TU IMAGEN) --- */}
          ...
          <tbody>
            {listadoTests.map((test, index) => (
              ...
              >
                ...
                {/* Columna Fallos: Indicador + Número */}
                ...
                {/* Columna Último (Fecha) */}
                <td className="px-6 py-4 text-lg text-center">{formatearFecha(test.ultimo_fecha)}</td>
                
                {/* Flecha > */}
                <td className="px-6 py-4">
                  <span className="text-3xl font-bold cursor-pointer hover:text-orange-500 transition-colors">›</span>
                </td>
              </tr>
            ))}
          </tbody>
